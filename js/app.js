(() => {
    ...
    document.addEventListener("click", (function(e) {
        ...
        languageItems.forEach((languageItem => {
            if (targetElement == languageItem) if (!languageItem.classList.contains("_active")) {
                languageItems.forEach((languageItem => {
                    languageItem.classList.remove("_active");
                }));
                languageItem.classList.add("_active");
            }
        }));
    }));
    ...
    if (calendar) {
        ...
        let isLeapYear = year => year % 4 === 0 && year % 100 !== 0 && year % 400 !== 0 || year % 100 === 0 && year % 400 === 0;
        let getFebDays = year => isLeapYear(year) ? 29 : 28;
        let generateCalendar = (month, year) => {
            ...
            if (null == month) month = currDate.getMonth();
            if (!year) year = currDate.getFullYear();
            let curr_month = `${month_names[month]}`;
            month_picker.innerHTML = curr_month;
            calendar_header_year.innerHTML = year;
            let first_day = new Date(year, month, 0);
            let last_day = new Date(year, month, days_of_month[month]);
            const firstDayIndex = first_day.getDay();
            const prevLastDay = new Date(currDate.getFullYear(), currDate.getMonth(), 0).getDate();
            const nextLastDay = new Date(last_day.getFullYear(), last_day.getMonth() + 1, 0).getDay();
            const nextDays = 7 - nextLastDay;
            let prevDay = "";
            for (let x = firstDayIndex; x > 0; x--) {
                prevDay += `<div class='prev-date'>${prevLastDay - x + 1}</div>`;
                calendar_days.innerHTML = prevDay;
            }
            for (let i = 0; i <= days_of_month[month] + first_day.getDay() - 1; i++) if (i >= first_day.getDay()) {
                let day = document.createElement("div");
                day.classList.add("calendar-day-hover");
                day.innerHTML = i - first_day.getDay() + 1;
                if (i - first_day.getDay() + 1 === currDate.getDate() && year === currDate.getFullYear() && month === currDate.getMonth()) day.classList.add("curr-date");
                calendar_days.appendChild(day);
            }
            let nextDay = "";
            for (let j = 1; j <= nextDays; j++) {
                if (7 == nextDays) break;
                nextDay = `<div class='next-date'>${j}</div>`;
                calendar_days.innerHTML += nextDay;
            }
        };
    ...
    function windowLoad() {
        ...
        let sections = document.querySelectorAll(".content-contactspage__item");
